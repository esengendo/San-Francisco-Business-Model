...
def categorize_project_type(description):
    ...
    if any(
        term in description
        for term in ["new construction", "new building", "new development"]
    ):
        ...
        term in description
        for term in ["renovation", "remodel", "rehab", "rehabilitation"]
    ):
        return "Renovation"
        ...
        term in description for term in ["change of use", "convert", "conversion"]
    ):
        ...
        term in description for term in ["interior", "tenant improvement", "ti work"]
    ):
...
def process_permits_data(df):
    ...
    permit_id_cols = [
        ...
        if any(
            x in col.lower()
            for x in ["permit_id", "permit_number", "application_id", "record_id"]
        )
    ]
    if permit_id_cols:
    ...
    date_columns = [
        ...
        if any(
            ...
            for date_term in ["date", "filed", "issued", "completed", "approved"]
        )
    ]
    ...
    status_col = next(
        (
            col
            for col in processed_df.columns
            if "status" in col.lower() and "date" not in col.lower()
        ),
        None,
    )
    if status_col:
        processed_df["status_category"] = processed_df[status_col].apply(
            categorize_status
        )
    ...
    cost_columns = [
        col
        for col in processed_df.columns
        if any(x in col.lower() for x in ["cost", "value", "fee", "price"])
    ]
...
def process_land_use_data(df):
    ...
    land_use_cols = [
        col
        for col in processed_df.columns
        if any(x in col.lower() for x in ["landuse", "land_use", "zoning", "use"])
    ]
    landuse_col = None
    ...
    if land_use_cols:
        ...
        if landuse_col:
            processed_df["land_use_category"] = processed_df[landuse_col].apply(
                categorize_land_use
            )
    ...
    important_cols = [
        ...
        "area_id",
        "property_name_2010",
        "map_block_lot_record",
        "min_zoning_height_ft",
        "max_zoning_height_ft",
        "building_median_height_m",
        "ground_min_elevation_m",
        "first_floor_median_height_m",
        "ground_to_first_floor_diff_m",
        "geometry",
        "land_use",
        "land_use_category",
        "data_as_of",
    ...
    existing_important_cols = [
        col for col in important_cols if col in processed_df.columns
    ]
...
def process_height_districts_data(df):
    ...
    for col in height_columns:
        try:
            ...
            processed_df[f"{col}_value"] = pd.to_numeric(
...
def create_aggregated_datasets(processed_planning_data, processed_dir, logger):
    ...
    permit_datasets = [
        name
        for name in processed_planning_data.keys()
    ...
    for permit_dataset in permit_datasets:
        try:
            ...
            if "year" in permits_df.columns and "neighborhood" in permits_df.columns:
                ...
                permit_trends_by_neighborhood = (
                    permits_df.groupby(["year", "neighborhood"])
                    .size()
                    .reset_index(name="permit_count")
                )
                save_to_parquet(
                    permit_trends_by_neighborhood,
                    f"{processed_dir}/planning",
                    f"{permit_dataset}_trends_by_year_neighborhood",
                )
                processed_planning_data[f"{permit_dataset}_trends_by_neighborhood"] = (
                    permit_trends_by_neighborhood
                )

                # By year and status
                if "status_category" in permits_df.columns:
                    permit_trends_by_status = (
                        permits_df.groupby(["year", "status_category"])
                        .size()
                        .reset_index(name="permit_count")
                    )
                    save_to_parquet(
                        permit_trends_by_status,
                        f"{processed_dir}/planning",
                        f"{permit_dataset}_trends_by_year_status",
                    )
                    processed_planning_data[f"{permit_dataset}_trends_by_status"] = (
                        permit_trends_by_status
                    )

                # By year and project type if available
                if "project_type" in permits_df.columns:
                    permit_trends_by_project = (
                        permits_df.groupby(["year", "project_type"])
                        .size()
                        .reset_index(name="permit_count")
                    )
                    save_to_parquet(
                        permit_trends_by_project,
                        f"{processed_dir}/planning",
                        f"{permit_dataset}_trends_by_year_project",
                    )
                    processed_planning_data[f"{permit_dataset}_trends_by_project"] = (
                        permit_trends_by_project
                    )
                ...
                if "month" in permits_df.columns:
                    ...
                    save_to_parquet(
                        monthly_trends,
                        f"{processed_dir}/planning",
                        f"{permit_dataset}_trends_monthly",
                    )
                    processed_planning_data[f"{permit_dataset}_trends_monthly"] = (
                        monthly_trends
                    )

                logger.info(
                    f"  - Created and saved {permit_dataset} trends aggregations"
                )
        except Exception as e:
    ...
    if "land_use" in processed_planning_data:
        try:
            ...
            if "land_use_category" in land_use_df.columns:
                ...
                land_use_summary = (
                    land_use_df["land_use_category"].value_counts().reset_index()
                )
                land_use_summary.columns = ["land_use_category", "count"]
                save_to_parquet(
                    land_use_summary, f"{processed_dir}/planning", "land_use_summary"
                )
...
def fetch_sf_planning_data(raw_data_dir, processed_dir, start_date):
    ...
    logger = logging.getLogger("SFBusinessPipeline.sf_planning_data_07")
    logger.info(
        f"Fetching SF Planning Department Data from {start_date.strftime('%Y-%m-%d')}..."
    )
    ...
    planning_datasets = [
        ...
        },  # Planning Projects - NEW (replaces kncr-c6jh)
        {
            "id": "y673-d69b",
            "name": "planning_non_projects",
        },  # Planning Non-Projects - NEW
    ...
    for dataset in planning_datasets:
        try:
            ...
            try:
                logger.info(
                    f"  - Attempting to load {dataset['name']} from existing raw file..."
                )
                raw_file_path = f"{raw_data_dir}/planning/{dataset['name']}_raw.parquet"
                df = pd.read_parquet(raw_file_path)
                all_planning_data[dataset["name"]] = df
                logger.info(
                    f"  - Loaded {len(df)} records from existing {dataset['name']}_raw.parquet"
                )
    ...
    for dataset_name, df in all_planning_data.items():
        try:
            ...
            if len(df.columns) != len(set(df.columns)):
                ...
                logger.warning(
                    f"Found duplicate columns in {dataset_name}: {list(duplicates)}"
                )
            ...
            if "permits" in dataset_name or "projects" in dataset_name:
                ...
                logger.info(
                    f"  - Processed and saved {dataset_name} data with {len(processed_df)} rows"
                )
        except Exception as e:
            ...
            try:
                ...
                save_to_parquet(
                    df_clean,
                    f"{processed_dir}/planning",
                    f"{dataset_name}_basic_processed",
                )
                logger.info(
                    f"  - Saved basic processed version of {dataset_name} with clean column names"
                )
            except Exception as basic_err:
                logger.error(
                    f"  - Could not even save basic processed version of {dataset_name}: {basic_err}"
                )
                processed_planning_data[dataset_name] = df

    # Create special aggregated datasets
    processed_planning_data = create_aggregated_datasets(
        processed_planning_data, processed_dir, logger
    )
...
if __name__ == "__main__":
    ...
    logger.info("Starting SF Planning data collection")
    logger.info(f"Base directory: {config['base_dir']}")
    
    # Create directories if they don't exist - same pattern as your _02 script
    os.makedirs(f"{config['raw_data_dir']}/planning", exist_ok=True)
    os.makedirs(f"{config['processed_dir']}/planning", exist_ok=True)

    # Execute the function
    planning_data = fetch_sf_planning_data(
        config["raw_data_dir"], config["processed_dir"], config["start_date"]
    )
    ...
    print(f"Files saved to: {config['processed_dir']}/planning/")
